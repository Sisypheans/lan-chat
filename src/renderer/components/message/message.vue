    ...
    <div class="message-wrapper" ref="list">
      <ul v-if="selectedChat">
        <li v-for="item in selectedChat.messages" class="message-item">
          ...
          <div class="message-main" :class="{ self: item.self }"> <!--判断是否是自己-->
...
<script>
  ...
  export default {
    ...
    mounted() {
      //  在页面加载时让信息滚动到最下面
      setTimeout(() => this.$refs.list.scrollTop = this.$refs.list.scrollHeight, 0) // scrollTop获取位于对象最顶端和窗口中可见内容的最顶端之间的距离  scrollHeight获取对象的滚动高度
    },
    watch: {
      // 发送信息后,让信息滚动到最下面
      messages() {
